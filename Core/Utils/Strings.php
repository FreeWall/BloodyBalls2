...
class Strings {
	...
	public static function match(string $subject,string $pattern,int $flags = 0,int $offset = 0):array{
		...
		return preg_match($pattern,$subject,$m,$flags,$offset)
			? $m
			: null;
	}
	...
	public static function parseURL($s,$follow = true):string {
		return preg_replace('/(?<![\"])https?:\/\/[\w\-\.!~#?&=+\*\'"(),\/]+/','<a href="$0" target="_blank"'.($follow ? '' : ' rel="nofollow"').'>$0</a>',$s);
	}

	public static function wordtruncate($s, $maxLen, $append = "\xE2\x80\xA6"):string {
	...
	public static function inflect($value,array $inflections):string {
		if(intval($value) == 1) return $inflections[0];
		else if(intval($value) >= 2 && intval($value) <= 4) return $inflections[1];
		return $inflections[2];
	}

	public static function safeBase64Encode($value):string {
		return rtrim(strtr(base64_encode($value),'+/','-_'),'=');
	}

	public static function safeBase64Decode($value):string {
		$value = base64_decode($value);
		return str_pad(strtr($value,'-_','+/'),strlen($value)%4,'=',STR_PAD_RIGHT);
	}

	public static function getLoremIpsum(int $maxLen,bool $dot = true):string {
		$lorems = array("lorem","ipsum","dolor","sit","amet","consectetur","adipiscing","elit","sed","do","eiusmod","tempor","incididunt","ut","labore","et","dolore","magna","aliqua","ut","enim","ad","minim","veniam","quis","nostrud","exercitation","ullamco","laboris","nisi","ut","aliquip","ex","ea","commodo","consequat","duis","aute","irure","dolor","in","reprehenderit","in","voluptate","velit","esse","cillum","dolore","eu","fugiat","nulla","pariatur");
		$content = ucfirst($lorems[rand(0,count($lorems)-1)]);
		while(strlen($content) < $maxLen){
			$content .= " ".$lorems[rand(0,count($lorems)-1)];
		}
		if(strlen($content) > $maxLen){
			...
			$content = implode(array_slice($parts,0,$last_part));
		}
		return trim($content).($dot ? "." : "");
	}
}